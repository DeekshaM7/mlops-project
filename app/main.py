...
def _load_model_and_transformer():
    ...
    if _MODEL is None or _TRANSFORMER is None:
        ...
        transformer_path = os.path.normpath(
            os.path.join(base, "..", "data", "processed", "transformer.joblib")
        )
...
def predict():
    ...
    if not data or "values" not in data:
        return (
            jsonify({"error": "JSON body must include 'values' (list of numbers)"}),
            400,
        )
        ...
        if isinstance(values, dict):
            ...
            if len(values) != len(FEATURE_COLUMNS):
                return (
                    jsonify(
                        {
                            "error": f"Expected {len(FEATURE_COLUMNS)} values (order: {FEATURE_COLUMNS}), got {len(values)}"
                        }
                    ),
                    400,
                )
            ...
            return (
                jsonify(
                    {
                        "error": "'values' must be a list or an object mapping feature name to value"
                    }
                ),
                400,
            )
