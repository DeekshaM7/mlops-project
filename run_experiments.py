...
def run_dvc_experiment(exp_config: Dict[str, Any]) -> bool:
    ...
    # Build DVC experiment command
    cmd = [
        "dvc", "exp", "run",
        "--name", exp_name,
        "--set-param", f"model.type={model_type}",
        "--set-param", f"train.n_trials={n_trials}",
        "--set-param", f"train.cv_folds={cv_folds}",
    ]
        ...
        try:
            ...
            if "permission denied" in error_msg or "errno 13" in error_msg:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"❌ Experiment {exp_name} failed after {max_retries} attempts!")
                    print(f"\n💡 FIX: Run this command to fix permissions:")
                    print(f"   .\\fix-dvc-permissions.ps1")
                    print(f"\nError: {e.stderr}")
                    return False
            else:
