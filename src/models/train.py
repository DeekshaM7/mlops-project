...
def load_splits(processed_dir: str, target_column: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def objective_rf(trial: optuna.Trial, X: np.ndarray, y: np.ndarray, cv_folds: int) -> float:
...
def objective_xgb(trial: optuna.Trial, X: np.ndarray, y: np.ndarray, cv_folds: int) -> float:
...
def train_model(model_type: str, X_train: np.ndarray, y_train: np.ndarray, 
                X_val: np.ndarray, y_val: np.ndarray, params: Dict[str, Any]) -> Tuple[Any, float, Dict[str, Any]]:
	...
	if model_type == "random_forest":
		study.optimize(lambda t: objective_rf(t, X_train, y_train, cv_folds), n_trials=n_trials)
		...
		study.optimize(lambda t: objective_xgb(t, X_train, y_train, cv_folds), n_trials=n_trials)
		best_params = study.best_params
