...
def parse_args():
    ...
    parser.add_argument(
        "--reason", default="Automated approval", help="Approval reason"
    )
    parser.add_argument(
        "--test-data", default="data/processed/test.csv", help="Path to test data"
    )
    parser.add_argument(
        "--min-accuracy", type=float, default=0.75, help="Minimum accuracy threshold"
    )
    parser.add_argument(
        "--min-precision", type=float, default=0.70, help="Minimum precision threshold"
    )
    parser.add_argument(
        "--min-recall", type=float, default=0.70, help="Minimum recall threshold"
    )
...
def run_bias_assessment(governance, model_name, version, test_data_path):
    ...
    try:
        ...
        bias_results = governance.assess_bias(
            model_name, version, test_data, protected_attributes
        )
...
def main():
    ...
    bias_ok, bias_results = run_bias_assessment(
        governance, args.model_name, args.version, args.test_data
    )
    ...
    if all_checks_passed:
        ...
        success = governance.approve_model(
            args.model_name, args.version, approver, args.reason
        )
        ...
        if success:
            ...
            report_path = (
                f"governance/reports/{args.model_name}-{args.version}-approval.json"
            )
        ...
        report_path = (
            f"governance/reports/{args.model_name}-{args.version}-rejection.json"
        )
        os.makedirs(os.path.dirname(report_path), exist_ok=True)
        with open(report_path, "w") as f:
