...
class ModelGovernance:
    ...
    def _load_compliance_rules(self):
        ...
        try:
            ...
            self.compliance_rules = {
                ...
                "required_documentation": ["model_card", "data_sheet", "performance_report"],
    ...
    def log_audit_event(self, event: AuditTrailEntry):
        ...
        logger.info(f"Audit event logged: {event.event_type} for {event.model_name}:{event.model_version}")
    ...
    def register_model(self, metadata: ModelMetadata) -> bool:
        ...
        try:
            ...
            # Save metadata
            metadata_path = f"governance/model-cards/{metadata.model_name}-{metadata.version}.json"
    ...
    def _register_mlflow_model(self, metadata: ModelMetadata):
        ...
        try:
            ...
            try:
                model_version = client.get_model_version(metadata.model_name, metadata.version)
                for key, value in tags.items():
                    client.set_model_version_tag(metadata.model_name, metadata.version, key, value)
            except mlflow.exceptions.RestException:
                logger.warning(f"Model version not found in MLflow: {metadata.model_name}:{metadata.version}")
                
        except Exception as e:
            logger.error(f"Failed to register model in MLflow: {str(e)}")
    
    def evaluate_compliance(self, model_name: str, model_version: str, 
                          performance_metrics: Dict[str, float],
        ...
        accuracy_check = {
            ...
            "status": "PASS" if performance_metrics.get("accuracy", 0.0) >= min_accuracy else "FAIL",
    ...
    def assess_bias(self, model, test_data: pd.DataFrame, 
                ...
                for value in unique_values:
                    ...
                    if mask.sum() > 0:
                        ...
                        group_metrics[str(value)] = {
                            ...
                            "count": int(mask.sum()),
                            "positive_prediction_rate": float((y_pred[mask] == 1).mean())
                        }
                
                # Calculate bias ratio (max difference in accuracy)
                accuracies = [metrics["accuracy"] for metrics in group_metrics.values()]
                bias_ratio = (max(accuracies) - min(accuracies)) if len(accuracies) > 1 else 0.0
        ...
        bias_report["bias_assessment"] = "LOW" if bias_report["max_bias_ratio"] < 0.05 else \
                                       "MODERATE" if bias_report["max_bias_ratio"] < 0.1 else "HIGH"
            ...
            with open(self.audit_log_path, 'r') as f:
                for line in f:
                    ...
                    if entry.get('model_name') == model_name:
                        if model_version is None or entry.get('model_version') == model_version:
